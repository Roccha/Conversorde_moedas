...
campo_moeda_destino = customtkinter.CTkOptionMenu(janela, values=["Selecione uma moeda de destino"])
...
def converter_moeda():
  ...
  texto_cotaçao_moeda.configure(text=f"1 {moeda_origem} = {cotacao} {moeda_destino}")
...
texto_cotaçao_moeda = customtkinter.CTkLabel(janela, text="")

moedas_disponiveis = nomes_moedas()
for codigo_moeda in moedas_disponiveis:
    nomes_moedas = moedas_disponiveis[codigo_moeda]
    texto_moeda = customtkinter.CTkLabel(lista_moeda, text=f"{codigo_moeda}:{nomes_moedas}")
    texto_moeda.pack()
...
lista_moeda.pack(padx=10, pady=10)
